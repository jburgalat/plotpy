...
The `panels` module provides :py:class:`plotpy.panels.PanelWidget` (the 
`panel` widget class from which all panels must derived from) and identifiers 
for each kind of panel:

    * :py:data:`plotpy.panels.ID_ITEMLIST`: ID of the `item list` panel
    * :py:data:`plotpy.panels.ID_CONTRAST`: ID of the `contrast 
      adjustment` panel
    * :py:data:`plotpy.panels.ID_XCS`: ID of the `X-axis cross section` 
      panel
    * :py:data:`plotpy.panels.ID_YCS`: ID of the `Y-axis cross section` 
      panel
