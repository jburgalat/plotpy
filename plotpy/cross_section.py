...
The `cross_section` module provides cross section related objects:
    * :py:class:`plotpy.cross_section.XCrossSection`: the X-axis 
      `cross-section panel`
    * :py:class:`plotpy.cross_section.YCrossSection`: the Y-axis 
      `cross-section panel`
...
from plotpy.interfaces import ICSImageItemType, IPanel, IBasePlotItem
from plotpy.panels import PanelWidget, ID_XCS, ID_YCS, ID_OCS
from plotpy.curve import CurvePlot, ErrorBarCurveItem
from plotpy.image import ImagePlot, LUT_MAX, get_image_from_qrect
from plotpy.styles import CurveParam
from plotpy.tools import ExportItemDataTool
from plotpy.geometry import translate, rotate, vector_norm, vector_angle
from plotpy.image import _scale_tr, INTERP_LINEAR
...
class CrossSectionItem(ErrorBarCurveItem):
    ...
    def set_source_image(self, src):
        ...
        (source: object with methods 'get_xsection' and 'get_ysection',
         e.g. objects derived from plotpy.image.BaseImageItem)
        """
